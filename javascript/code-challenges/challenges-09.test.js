...
const getHouses = (arr) => {
  let houses = [];
  //I'm not really sure how keys, values or entries would help here? I think something "return arr.map(p => p.house)" would be best.
...
const hasChildrenValues = (arr, character) => {
  return Object.values(arr.filter(p => p.name === character)[0]).length > 3;
  //my solution given the requirements is above, but I think the following is more proper:
  //return 'children' in arr.find(person => person.name === character);
};
...
const hasChildrenEntries = (arr, character) => {
  //NOTE: hasChildrenValues is not challenge 4
  const ch = arr.find(person => person.name === character);
  if (ch === undefined){
    return false;
  }
