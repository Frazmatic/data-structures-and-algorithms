...
const sortByChildren = (charArray) => {
  return charArray.sort((a, b) => {
    ...
    if ([a.children.length, a.house] < [b.children.length, b.house]){
      return -1;
    }
    // eslint-disable-next-line no-constant-condition
    else if ([a.children.length, a.house] > [b.children.length, b.house]){
      return 1;
    }
    else {
      return 0;
...
const isCapitalized = (str) => {
  //I don't quite understand how "?" works but it made this work
  // googled 'find words in regex' and discovered /b word boundary
  let regex = /\b[A-Z].*?\b/g;
...
const matchMonth = (input) => {
  //well it's not *in* the regex
  return /^[Oo]ct$/.test(input) || /^[Oo]ctober$/.test(input);
};
