...
const notInFirstArray = (forbiddenValues, arr) => {
  // this would just be "return arr.difference(forbiddenValues)"" in python!
  return arr.filter(el => !forbiddenValues.includes(el));
  // I found includes() while looking how to do search for the lab's stretch goal; otherwise could loop through 2nd array and return false if match
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(el => el.baseStat > minBaseStat).map(stat => stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  //if the key doesn't exist this willy coerce to false (falsey) and ! will make it true
...
const evenOddNumericValues = (arr) => {
  return arr.filter(el => typeof el === 'number').map(el => el % 0 ? 'odd' : 'even');
};
