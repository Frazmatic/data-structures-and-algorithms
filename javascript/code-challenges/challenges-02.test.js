...
const raisedToTheThird = (arr) => {
  let newArr = [];
  arr.forEach(el => { newArr.push(Math.pow(el, 3)); });
  return newArr;
};
...
const forEachTwoToThe = (arr) => {
  let output = [];
  arr.forEach(n => { output.push(Math.pow(2, n)); });
  return output;
};
...
const charCode = (arr) => {
  return arr.map(ch => ch.charCodeAt(0));
};
...
const evenOdd = (arr) => {
  return arr.map(n => typeof n === 'number' ? (n % 2 ? 'odd' : 'even') : 'N/A');
};

//how do I get the tester to not skip the stretch goal tests? I see the tests are written.
...
const extractStats = (arr) => {
  return arr.map(s => { return {name: s.stat.name, total: s.effort + s.baseStat}; });
};
